...
import { updateBookingSettingSchema } from "@/modules/restaurant/restaurant.schema";
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const parsed = updateBookingSettingSchema.safeParse(await request.json());
    if (!parsed.success) {
      return fail(ERROR_CODES.VALIDATION_ERROR, parsed.error.issues[0]?.message ?? MESSAGES.restaurant.updateFailed, 400);
    }

    const { confirm, ...settings } = parsed.data;
    const result = await restaurantService.updateBookingSetting(id, profile.id, settings, confirm);
    if (!result.success) {
